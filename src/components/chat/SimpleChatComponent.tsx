...
export const SimpleChatComponent = () => {
  ...
  const fetchOnlineUsers = async () => {
    try {
      ...
      // Combinar dados
      const users = presenceData.map((presence: any) => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const messagesWithProfiles = messagesData.map((message: any) => ({
        ...message,
        user_profile: profilesData?.find((p: any) => p.id === message.user_id) || null
      }));
              ...
              <div className="space-y-4 py-4">
                <AnimatePresence initial={false}>
                  {messages.map((message) => {
                    ...
                    const socialName = message.user_profile?.social_name || 'Usuário';
